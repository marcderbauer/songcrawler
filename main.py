...
parser.add_argument("--verbose", default=False, action='store_true', help="Detailed output. Lists alternative Genius ids when main argument is a songname")
...
def parse_main_argument(arg):
    # Differentiates whether the main argument is a uri, songname or genius_id
    # no need to use regex for this, can just use inbuilt methods
    # Spotify_ids start with spotify:
    # genius_ids are numbers -> isdigit() == True
    # everything else is a songname
    pass

def main():
    parse_main_argument(args.main_argument)

class Songcrawler():
    def __init__(self, get_lyrics=True, filetype="json", use_genius_album=False) -> None:
        # TODO:
        # uri shouldn't be bound to object
        # creating object and requesting something should be two seperate steps
        ...
        self.no_lyrics = {} # trackname: spotify_uri for songs without lyrics

    def request(self, uri, genius_id=None, get_lyrics=True):
        request_type = self.get_request_type(uri)
        self.get_lyrics = get_lyrics # does this make sense. So the get_lyrics param doesn't need to get passed down
        match request_type:
            case "track":
                return self.get_song(uri, genius_id, get_lyrics)
            case "album":
                return self.get_album(uri, get_lyrics)
            case "artist":
                return self.get_artist(uri, get_lyrics)
            case "playlist":
                return self.get_playlist(uri, get_lyrics)
            case _:
                raise Exception(f'Unknown request type: \"{request_type}\"')

    def get_request_type(self, uri):
        """
    ...
    def get_song(self, song_uri, genius_id=None, get_lyrics=True): # num retries should be part of the CLI
    ...
    def get_album(self, album_uri, get_lyrics=True): # TODO: flag for using genius albums?
        ...
        if get_lyrics:
            ...
            songs = {name:self.get_song(uri, get_lyrics=False) for name, uri in songs_to_uri.items()}
            return songs



    def get_artist(self, artist_uri, regex_filter = None, album_type="album", get_lyrics=True):
...
if __name__=="__main__":
    sc = Songcrawler()
    sc.request("spotify:track:5CBEzaNEuv3OO32kZoXgOX")
