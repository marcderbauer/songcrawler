...
class View():
    ...
    def _overwrite_prompt(self, resource_name, resource_type, ask_multiple=False):
        if ask_multiple:
            overwrite = Prompt.ask(
                f"\n{resource_type} \"{resource_name}\" already exists. Would you like to overwrite it? ([y]es/[n]o/[a]lways)",
                choices=["y","n","a"]
            )
        else:
            overwrite = Confirm.ask(
                f"\n{resource_type} \"{resource_name}\" already exists. Would you like to overwrite it?"
            )
